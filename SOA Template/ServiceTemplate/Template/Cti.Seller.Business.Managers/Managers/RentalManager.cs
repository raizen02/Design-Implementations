...
namespace Cti.Seller.Business.Managers
{
    ...
    public class RentalManager : ManagerBase, IRentalService
    {
        ...
        public Rental RentCarToCustomer(string loginEmail, int carId, DateTime dateDueBack)
        {
            ...
            {
                ICarRentalEngine SellerEngine = _BusinessEngineFactory.GetBusinessEngine<ICarRentalEngine>();

                try
                {
                    Rental rental = SellerEngine.RentCarToCustomer(loginEmail, carId, DateTime.Now, dateDueBack);
        ...
        public Rental RentCarToCustomer(string loginEmail, int carId, DateTime rentalDate, DateTime dateDueBack)
        {
            return ExecuteFaultHandledOperation(() =>
            {
                ...
                try
                {
                    Rental rental = RentalEngine.RentCarToCustomer(loginEmail, carId, rentalDate, dateDueBack);
        ...
        public void ExecuteRentalFromReservation(int reservationId)
        {
            ExecuteFaultHandledOperation(() =>
            {
                ...
                try
                {
                    Rental rental = RentalEngine.RentCarToCustomer(account.LoginEmail, reservation.CarId, reservation.RentalDate, reservation.ReturnDate);
                }
                catch (UnableToRentForDateException ex)
                {
                    throw new FaultException<UnableToRentForDateException>(ex, ex.Message);
        ...
        public bool IsCarCurrentlyRented(int carId)
        {
            return ExecuteFaultHandledOperation(() =>
            {
                ...
                return RentalEngine.IsCarCurrentlyRented(carId);
            });
        }
