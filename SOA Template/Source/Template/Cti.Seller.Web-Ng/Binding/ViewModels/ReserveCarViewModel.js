...
var reserveCarModule = angular.module('reserveCar', ['common'])
    .config(function ($routeProvider, $locationProvider) {
        $routeProvider.when(Seller.rootPath + 'customer/reservecar', { templateUrl: Seller.rootPath + 'Templates/Reserve.html', controller: 'ReserveViewModel' });
        $routeProvider.when(Seller.rootPath + 'customer/reservecar/carlist', { templateUrl: Seller.rootPath + 'Templates/CarList.html', controller: 'CarListViewModel' });
        $routeProvider.otherwise({ redirectTo: Seller.rootPath + 'customer/reservecar' });
...
reserveCarModule.controller("ReserveCarViewModel", function ($scope, $window, viewModelHelper) {
    ...
    $scope.reserveCarModel = new Seller.ReserveCarModel();
...
reserveCarModule.controller("ReserveViewModel", function ($scope, $http, $location, viewModelHelper, validator) {
    ...
    $scope.submit = function () {
        ...
        if (viewModelHelper.modelIsValid) {
            ...
            $location.path(Seller.rootPath + 'customer/reservecar/carlist');
        }
        else
            viewModelHelper.modelErrors = $scope.reserveCarModel.errors;
    }
...
reserveCarModule.controller("CarListViewModel", function ($scope, $http, $location, viewModelHelper) {
    ...
    $scope.availableCars = function () {
        if (!$scope.reserveCarModel.initialized) {
            $location.path(Seller.rootPath + 'customer/reservecar');
        }
        else {
